...
export default new Vuex.Store({
	modules: {
		pizza: {
			...
			actions: {
				getPizzaSize({ commit }, payload) {
					commit('setPizzaSize', payload);
				},
				getPizzaFilling({ commit }, payload) {
					commit('setPizzaFilling', payload);
				},
				getPizzaDough({ commit }, payload) {
					commit('setPizzaDough', payload);
				},
				controlSteps({ commit }, payload) {
					commit('controlSteps', payload);
				},
				// eslint-disable-next-line
				async getPizzasSizes({ commit }, payload) {
					try {
						const { data } = await Provider.get('/sizes');
				...
				// eslint-disable-next-line
				async getPizzasFillings({ commit }, payload) {
					try {
						const { data } = await Provider.get('/fillings');
				...
				// eslint-disable-next-line
				async getPizzasDoughs({ commit }, payload) {
					try {
						const { data } = await Provider.get('/doughs');
				...
				// eslint-disable-next-line
				async getPizzasInPromotion({ commit }, palyload) {
					try {
						const { data } = await Provider.get('/product/promotion');
				...
				async paymentPromtionPizza(
					...
					// eslint-disable-line
					try {
						const { data } = await Provider.post('/product/order-promotion', {
							id: id,
						});
				...
				async paymentPizza(
					...
					// eslint-disable-line
					try {
						const { data } = await Provider.post('/product/order', {
							id: id,
						});
						commit('setOrderSuccess', data);
						commit('resetPizza', {});
